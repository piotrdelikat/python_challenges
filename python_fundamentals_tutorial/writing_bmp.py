...
def write_grayscale(filename, pixels):
    ...
    with open(filename, 'wb') as bmp:
        ...
        size_bookmark = bmp.tell()  # Next four bytes hold the filesize as a 32-bit
        bmp.write(b'\x00\x00\x00\x00')  # Zero placeholder

        bmp.write(b'\x00\x00')  # Unused 16-bit integer - should be zero
        bmp.write(b'\x00\x00')  # Unused 16-bit integer - should be zero

        pixel_offset_bookmark = bmp.tell()  # Next four bytes hold the integer offset
        ...
        for row in reversed(pixels):  # As BMP files are bottom to top
            row_data = bytes(row)
            bmp.write(row_data)
            padding = b'\x00' * (4 - (len(row) % 4))  # Padding row to multiple of four bytes
            bmp.write(padding)
